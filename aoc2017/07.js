...
input.split('\n').forEach(line => {
    ...
    if (match) {
        ...
        const list = match[4] && match[4].split(', ');
...
function sum(node) {
    ...
    let t = 0;
    const map = {};
    const childSums = node.children.map(child => {
        t = sum(child);
        s += t;
    ...
    const correct = childSums.filter(sum => map[sum] > 1)[0];

    if (node.children.length > 1) {
        node.children.forEach(child => {
            if (map[child.sum] === 1) {
                console.log(child)
                console.log(correct - child.sum + child.weight);
                found = true;
            }
        })
    }
