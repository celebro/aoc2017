const input = `utc dec -736 if p > -7
tn inc -876 if qlm == 4
uz dec 294 if l < 10
a inc -904 if me >= -7
tn inc 622 if ppl <= 6
...
input.split('\n').forEach((line, ix) => {
    const [register, op, value, _, condReg, condOp, condValue] = line.split(' ');

    if (condOps[condOp](registers[condReg] || 0, +condValue)) {
        registers[register] = (registers[register] || 0) + (op === 'inc' ? (+value) : -(+value));
